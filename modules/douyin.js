...
export function map_item(item) {
    ...
    if (item["ZS_collected_from_embed"]) {
        ...
        if (Object.keys(stream_data).length) {
            ...
            video_thumbnail = py_get(stream_data["video"] ?? {}, "cover");
            video_description = stream_data["title"] ?? "";
            duration = "Unknown";
            prevent_download = new MissingMappedField("Unknown");
            ...
            post_timestamp = first_timestamp([item["createTime"], 1]);
            ...
            video_thumbnail = py_get(item["video"] ?? {}, "cover");
        ...
        const collect_count = value_or_missing(stats, "collectCount", "Unknown");
        const comment_count = value_or_missing(stats, "commentCount", "Unknown");
        const digg_count = value_or_missing(stats, "diggCount", "Unknown");
        const download_count = value_or_missing(stats, "downloadCount", "Unknown");
        const forward_count = value_or_missing(stats, "forwardCount", "Unknown");
        const play_count = value_or_missing(stats, "playCount", "Unknown");
        const share_count = value_or_missing(stats, "shareCount", "Unknown");
        ...
        mix_info = defined(item, mix_info_key, {}) || {};
        ...
        if (stream_data) {
            ...
            subject = "Stream";
            // Some posts carry no timestamp; the time of collection stands in
            post_timestamp = first_timestamp(
                [stream_data["create_time"], 1],
                [item["create_time"], 1],
                [metadata["timestamp_collected"], 1000]
            );
            video_url = stream_link(stream_data);
            video_thumbnail = py_get(stream_data["video"] ?? {}, "cover");
            video_description = stream_data["title"] ?? "";
            ...
            // Regular post
            post_timestamp = first_timestamp([item["create_time"], 1]);
        ...
        const collect_count = Object.keys(stats).length ? py_get(stats, "collect_count") : new MissingMappedField("Unknown");
        const comment_count = Object.keys(stats).length ? py_get(stats, "comment_count") : new MissingMappedField("Unknown");
        const digg_count = Object.keys(stats).length ? py_get(stats, "digg_count") : new MissingMappedField("Unknown");
        const download_count = Object.keys(stats).length ? py_get(stats, "download_count") : new MissingMappedField("Unknown");
        const forward_count = Object.keys(stats).length ? py_get(stats, "forward_count") : new MissingMappedField("Unknown");
        const play_count = Object.keys(stats).length ? py_get(stats, "play_count") : new MissingMappedField("Unknown");
        const share_count = Object.keys(stats).length ? py_get(stats, "share_count") : new MissingMappedField("Unknown");
        const videoTagRaw = defined(item, "video_tag");
        if (Array.isArray(videoTagRaw)) {
    ...
    const music_author = py_get(music_obj, "author", "");
    const music_title = py_get(music_obj, "title", "");
    const music_url = Object.keys(music_obj).length ? py_get(music_obj["play_url"] ?? {}, "uri") : "";
    ...
    const collection_id = defined(mix_info, mix_id_key, "N/A");
    const collection_name = defined(mix_info, mix_name_key, "N/A");
    const part_of_collection = (mix_id_key in mix_info && collection_id !== "N/A") ? "yes" : "no";
    ...
    return new MappedItem({
        ...
        "author_user_id": (author_id_key in item) ? item[author_id_key] : py_get(author, "uid", py_get(author, "id")),
