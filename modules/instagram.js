...
export function map_item(item) {
    ...
    if ((item.product_type === 'ad') || (link && link.startsWith('https://www.facebook.com/ads/ig_redirect'))) {
        throw new Error('appears to be Instagram ad, check raw data to confirm and ensure Zeeschuimer is up to date.');
    }
...
function parse_polaris_item(node) {
    ...
    if (user && owner) {
        if (owner.id === user.id) {
            // Same id; owner may contain less info, so prefer user
        } else if (user.username !== owner.username) {
            throw new Error('Unable to parse item: different user and owner');
        }
    }
...
function parse_graph_item(node) {
    ...
    const num_media = node.__typename !== 'GraphSidecar' ? 1 : (node.edge_sidecar_to_children ? node.edge_sidecar_to_children.edges.length : 0);
    ...
    let play_count;
    if (node.view_count !== undefined) {
        play_count = node.view_count;
    } else if (node.play_count !== undefined) {
...
function parse_itemlist_item(node) {
    const partial_item = node._zs_partial ?? false;
    const num_media = node.media_type !== 8 ? 1 : (node.carousel_media ? node.carousel_media.length : 0);
    const caption = 'caption' in node ? (node.caption ? node.caption.text : '') : new MissingMappedField('');
        ...
        if (media_node.media_type === 2) {
            ...
            if ('image_versions2' in media_node) {
                ...
                if (partial_item) {
                    ...
                    throw new Error('Instagram item format change');
                }
            }
    ...
    const usertags = 'usertags' in node ? node.usertags.in.map(user => user.user.username).join(',') : '';

    let collected_at, unix_at;
    if (partial_item) {
        // Missing data
