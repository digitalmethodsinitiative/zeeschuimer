...
function parsePolarisItem(node) {
    ...
    if (node.user && node.owner) {
        if (owner.id === user.id) {
            // prefer user, nothing to do
...
function parseGraphItem(node) {
    ...
    if (node.__typename !== "GraphSidecar") {
        ...
        if (mediaTypes.size > 1) {
            ...
            const single = mediaTypes.values().next().value;
    ...
    const usertags = (node.edge_media_to_tagged_user?.edges ?? []).map(e => e.node.user.username).join(",");
...
function parseItemlistItem(node) {
    ...
    for (const media_node of media_nodes) {
        ...
        mediaTypes.add(typeMap[media_node.media_type] ?? "unknown");
    }

    const media_type = mediaTypes.size > 1 ? "mixed" : (mediaTypes.values().next().value);
    ...
    if (Array.isArray(node.coauthor_producers)) {
        for (const coauthor of node.coauthor_producers) {
            coauthorsArr.push(coauthor.username ?? new MissingMappedField(""));
            coauthorFullnamesArr.push(coauthor.full_name ?? new MissingMappedField(""));
            coauthorIdsArr.push(coauthor.id);
        }
    }
    ...
    return {
        ...
        "play_count": play_count,
        "likes_hidden": no_likes ? "yes" : "no",
        "num_likes": !no_likes ? node.like_count : new MissingMappedField(0),
...
export function map_item(item) {
    ...
    const isPolaris = ("__typename" in item) && item.__typename && item.__typename.toLowerCase().includes("polaris");
    const isGraph = ("__typename" in item) && item.__typename !== "XDTMediaDict";
